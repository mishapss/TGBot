...
async def send_notification(bot, chat_id, task_title, when):
    ...
    try:
        ...
        #print(f"Уведомление отправлено для {task_title}")
    except Exception as e:
    ...
    with psycopg2.connect(**DB_CONFIG) as conn:
        with conn.cursor() as cursor:
            ...
            for task in tasks:
                ...
                #print(f"\nЗадача: {title} | Напоминание: {reminder_type} deadline {due_date}")
                ...
                if reminder_type.startswith("В этот день в"):
                    try:
                        db_time = reminder_type.split("в ")[1].split()[0]  # "15:42"
                        if db_time == current_time:
                            #print(f"✅ Совпадение времени: {db_time}")
            ...
            if current_time == "09:00":
                ...
                #print(f"дата для проверки за 1 день до {one_day_start}")
                #print(f"Найдено задач за 1 день: {len(tasks_1_day)}")
                #print(tasks_1_day)

                for task in tasks_1_day:
                    task_id, title, assignee, chat_id = task
                    #print(f"обрабатываем задачу {title} {task_id}")
            ...
            if current_time == "09:00":
                ...
                #print(f"дата для проверки за 2 дня до {two_days_start}")
                #print(f"Найдено задач за 2 дня: {len(tasks_2_days)}")
                #print(tasks_2_days)
